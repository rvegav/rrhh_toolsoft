	...
	<div class="row">
		<div class="col-md-12 col-sm-12 col-xs-12">
			<div class="col-md-12" align="right">
				<a href="<?php echo base_url();?>tiposalarios/tiposalarios/add" class="btn btn-dark">
					<i class="fa fa-plus">
					</i> Agregar Tipo de Salario
				</a>
				<?php
			...
			<div class="x_panel">
				...
				<div class="row">
					<div class="col-md-12">
						<table id="tb_tipoSalario" class="table table-striped table-bordered btn-hover" width="100%">
							<thead>
								<tr>
									<th>#</th>
									<th>Código</th>
							...
							<tbody>
								<?php
								if(!empty($tiposalarios)):?>
								<?php
								foreach($tiposalarios as $tiposalario):?>
								<tr>
									<td>
										<?php echo $tiposalario->IDTIPOSALARIO;?>
									</td>
									<td>
										<?php echo $tiposalario->NUMTIPOSALARIO;?>
									</td>
									<td>
										<?php echo $tiposalario->DESTIPOSALARIO;?>
									</td>
									<td>
										<?php echo $tiposalario->IMPORTE;?>
									</td>
									<td>
										<?php echo $tiposalario->FECGRABACION;?>
									</td>
									...
									<td>
										...
										<button type="button" class="btn btn-primary btn-view" data-toggle="modal" data-target="#modal-view" value="<?php echo $tiposalario->IDTIPOSALARIO;?>">
											<i class="fa fa-eye">
											</i>
										</button>
										<a href="<?php echo base_url();?>tiposalarios/tiposalarios/edit/<?php echo $tiposalario->IDTIPOSALARIO;?>" class="btn btn-warning">
											<i class="fa fa-edit">
											</i>
										</a>
										<a href="#" id="<?=$tiposalario->IDTIPOSALARIO;?>" class="btn btn-danger btn-delete eliminar">
											<i class="fa fa-trash">
											</i>
										</a>
	...
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title" id="myModalLabel"><i class='fa fa-eye'></i> Ver Detalles del Tipo de Salario </h4>
		  	</div>
...
<script>
 $(document).ready(function(){
 	$('#tb_tipoSalario thead tr').clone(true).appendTo( '#tb_tipoSalario thead' );
		$('#tb_tipoSalario thead tr:eq(1) th').each( function (i) {
		...
		var table = $('#tb_tipoSalario').DataTable({
			dom: 'Bfrtip',
			"columnDefs": [
			{ "width": "20%", "targets": 6 }],
			"orderCellsTop": true,
			"buttons": [{
				extend: 'pdfHtml5',
				text: 'Generar PDF',
				title:'Lista de Tipo de Salario' ,
				exportOptions: {
                    columns: [ 0, 1, 2, 3, 4 ]
                }, 
 	...
 	var base_url= "<?php echo base_url();?>";
       // alert (base_url);
        $(".btn-view").on("click", function(){
          var id= $(this).val();
          $.ajax({
            url: base_url + "tiposalarios/tiposalarios/view/" + id,
            type: "POST",
 ...
 function eliminar__(id){
	if(confirm("Esta seguro que desea eliminar este registro?")){
		...
		window.location.href = "/isupport/tiposalarios/tiposalarios/delete/" + id;
