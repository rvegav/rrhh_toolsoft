...
<div class="right_col" role="main">
	...
	<div class="x_panel">
		...
		<div class="row">
			<div class="col-md-12">
				<table id="tb_tipoMoviemiento" class="table table-bordered btn-hover" style="border-collapse:collapse; " width="100%">
					<thead>
						<tr>
							<th>#</th>
							<th>Codigo</th>
					...
					<tbody>
						...
						<tr data-toggle="collapse" data-target="#demo<?php echo $tipo->IDTIPOMOVISUELDO;?>" class="accordion-toggle">
							<td>
								<i class="fa fa-plus-circle botonmas">									
								</i>
							</td>
							<td>
								<?php echo $tipo->NUMTIPOMOV;?>
							</td>
							<td>
								<?php echo $tipo->DESTIPOMOV;?>
							</td>
							<td>
								<?php echo $tipo->SUMARESTA;?>
							</td>
							<td>
								<?php echo $tipo->CUENTACONTABLE;?>
							</td>
								<td>
									<button type="button" class="btn btn-primary btn-view" data-toggle="modal" data-target="#modal-view" value="<?php echo $tipo->IDTIPOMOVISUELDO;?>">
										<i class="fa fa-eye">
										</i>
									</button> 


									<a href="<?php echo base_url();?>tipomovimientos/tipomovimientos/edit/<?php echo $tipo->IDTIPOMOVISUELDO;?>" class="btn btn-warning">
										<i class="fa fa-edit">
										</i>
									</a>
									<a href="#" onclick="eliminar(<?php echo $tipo->IDTIPOMOVISUELDO; ?>);" class="btn btn-danger btn-delete">
							...
							<tr>
								<td colspan="6"  class="hiddenRow">
									<div id="demo<?php echo $tipo->IDTIPOMOVISUELDO;?>" class="accordian-body collapse">
										<table class="table">
											<thead>
												<tr>													
													<th>
														Movimientos para Calculo
													</th>									
												</tr>
											</thead>
												...
												<?php foreach($detalles as $detalle):?>
													<tr>										
														<?php if(!empty($tipos)):?>
															...
															<td>
																<?php if($detalle->IDTIPOMOVIDETALLE == $tipo->IDTIPOMOVISUELDO):?>
																	<div>
																		<?php echo trim($detalle->DESCRIPCION);?>
																	</div> 
																<?php endif; ?>
															</td>
														<?php endif; ?>
	...
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title" id="myModalLabel"><i class='fa fa-eye'></i> Ver Detalles de Tipo de Movimientos </h4>
			</div>
...
<script type="text/javascript">
	$(document).ready(function(){
		$('#tb_tipoMoviemiento thead tr').clone(true).appendTo( '#tb_tipoMoviemiento thead' );
		$('#tb_tipoMoviemiento thead tr:eq(0) th').each( function (i) {
			if (i=='0') {
				console.log($(this));
			}
		});

		$('#tb_tipoMoviemiento thead tr:eq(1) th').each( function (i) {
		...
		var table = $('#tb_tipoMoviemiento').DataTable({
			dom: 'Bfrtip',
			"columnDefs": [
			{ "width": "20%", "targets": 5 }],
			"orderCellsTop": true,
			"buttons": [{
				...
				customize: function ( doc ) {
					...
					$.each(headers ,function( index){
						console.log(index);
						doc.content[1].table.body[0][index].fillColor = '#11263b'
					});
		...
		$(".botonmas").on("click", function(){
			$(this).removeClass("fa fa-plus-circle botonmas");
    		$(this).addClass("fas fa-minus-circle botonmenos");	
		});

		
		 $(document).click(".botonmenos", function(){			
       ...
       $(".btn-view").on("click", function(){
       	var id= $(this).val();
       	$.ajax({
       		url: base_url + "tipomovimientos/tipomovimientos/view/" + id,
       		type: "POST",
  ...
  //        $(".btn-warning").on("click", function(e){
	...
	function eliminar(id){
		if(confirm("Esta seguro que desea eliminar este registro?")){
			...
			window.location.href = "/isupport/tipomovimientos/tipomovimientos/delete/" + id;
